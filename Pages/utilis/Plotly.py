...
def filter_data(dataframe,num_period):
    if num_period == '1mo':
        data = dataframe.index[-1] + dateutil.relativedelta.relativedelta(months=-1)
    elif num_period == '5d':
        data = dataframe.index[-1] + dateutil.relativedelta.relativedelta(days=-5)
    elif num_period == '3mo':
        data = dataframe.index[-1] + dateutil.relativedelta.relativedelta(months=-3)
    elif num_period == '6mo':
        data = dataframe.index[-1] + dateutil.relativedelta.relativedelta(months=-6)
    elif num_period == 'ytd':
        data = datetime(datetime.now().year,1,1)
    elif num_period == '1y':
        data = dataframe.index[-1] + dateutil.relativedelta.relativedelta(years=-1)
    elif num_period == '5y':
        data = dataframe.index[-1] + dateutil.relativedelta.relativedelta(years=-5)
    elif num_period == 'max':
        data = dataframe.index[0]

    return dataframe.reset_index()[dataframe.reset_index()['Date'] >= data]
